...
class RNN_VAE(nn.Module):
    ...
    def sample_z(self, mu, logvar):
        ...
        """
        eps = Variable(torch.randn(self.z_dim)) # Why sample from N(0,I) only once? mu.size(0),
        eps = eps.cuda() if self.gpu else eps
        return mu + torch.exp(logvar/2) * eps # Why divide by 2?
    ...
    def generate_soft_embed(self, mbsize, temp=1):
        """
        Generate soft embeddings of (mbsize x seq_len x emb_dim) along with target z
        ...
        for _ in range(mbsize):
            ...
            samples.append(self.sample_soft_embed(z, c, temp=temp))  # Replace 1 with temp?
            targets_z.append(z)
            targets_c.append(c)
